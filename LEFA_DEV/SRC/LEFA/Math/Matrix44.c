...
matrix4_get_basis (const GMatrix4 self, int basis)
{
    switch (basis)
    {
        case 0: return vec3_init(self.m[0][0], self.m[1][0], self.m[2][0]);
        case 1: return vec3_init(self.m[0][1], self.m[1][1], self.m[2][1]);
        case 2: return vec3_init(self.m[0][2], self.m[1][2], self.m[2][2]);
    }
