...
Quaternion 
quaternion_init_vectors(GVector3 v1,
                        GVector3 v2)
{
    Quaternion self;
    GVector3 c;

    c = vector3_cross (v1, v2);
    self.x = c.x;
    self.y = c.y;
    self.z = c.z;
    self.w = sqrt(vector3_dot (v1, v1) + vector3_dot (v2, v2)) + vector3_dot (v1, v2);
...
Quaternion 
quaternion_lookat(GVector3 position,
                  GVector3 center,
                  GVector3 up)
{
    GVector3 direction;
...
float
quaternion_get_rotation(const Quaternion self, GVector3* axis)
{
    if (axis != 0) //0 = NULL
        *axis = vector3_normalize(vector3_init(self.x, self.y, self.z));
